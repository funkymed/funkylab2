 ...
 * =================================================================================================
 *	Mentions légales
 *  Funkylab est entiérement réalisé en Php/Mysql & Javascript
...
//==================================================================================================
	...
	$buffer_template_HTML=file_get_contents(CURRENT_SKIN."/template.html");
 	$buffer_template_HTML=str_replace("images/",CURRENT_SKIN."/images/",$buffer_template_HTML);
	$buffer_template_HTML=str_replace("css/",CURRENT_SKIN."/css/",$buffer_template_HTML);
	$tableglobal="";
...
//==================================================================================================
//	MENU (Si un ou plusieurs #nom# de menu est pr�sent dans le template il est affiché)
//==================================================================================================
		...
		while ($row = mysql_fetch_array($res)){
			if (strstr ($buffer_template_HTML,stripslashes($row['nom']))){
				$menuHTML.=Generate_Menu::makemenuhtml($row['id']);
			 	...
			 	$buffer_template_HTML=str_replace(stripslashes($row['nom']),$menuHTML,$buffer_template_HTML);
	 		}
		}
...
//==================================================================================================
	...
	//==================================================================================================
	//	CHEMIN DE FER (Si une variable de type cat, page, galerie est invoqu� elle serat affich� ici)
...
//==================================================================================================
	...
	if (isset($_GET['galerie'])){
		...
		$tableglobal=str_replace("#TITRE#","<a href='index.php?galerie=$galerieid'>".$titregalerie."</a>",$tableglobal);
		$tableglobal=str_replace("#CONTENU1#",$page,$tableglobal);
		$tableglobal=str_replace("#INFO#","",$tableglobal);
...
//==================================================================================================
	...
	<form action\"index.php\" method=\"GET\">
		<fieldset>
			...
			<p>Pour vous abonner ou vous d�sabonner de notre newsletter, entrez votre email ci-dessous et cliquez sur 'ok'</p>
					...
					<fieldset>
						...
						if (verifmail($email)==true){
							...
							if (checkifexiste($mail)==true){
								...
								$tableglobal.=
								"Vous venez de vous d�-inscrire de notre newsletter.";
							}else{
								$query="INSERT INTO mail_newsletter (id,mail,date,IPcreation) VALUES ('','$mail','$date','$IPcreation')";
								$resultat=mysql_query($query);
								$tableglobal.=
								"
									<P>Vous venez de vous inscrire � notre newsletter.</P>
									<P>Si vous desirez vous desinscrire entrez � nouveau votre email dans la zone newsletter � gauche.</P>
								";
							}
						}else{
							$tableglobal.=
							"
								...
								<P>Essayez de le saisir � nouveau.</P>
							";
						}
						$tableglobal.="
					</fieldset>
...
//==================================================================================================
	...
	$buffer_template_HTML=str_replace("#RECHERCHE#",$finder,$buffer_template_HTML);
...
//==================================================================================================
	...
	$buffer_template_HTML=str_replace("#RSS#",$rsspdf,$buffer_template_HTML);
//==================================================================================================
...
//==================================================================================================
	...
	$buffer_template_HTML=str_replace("#PUB#",$pub,$buffer_template_HTML);
...
//==================================================================================================
	...
	while ($row = mysql_fetch_array($res)){
		...
		if (strstr ($buffer_template_HTML,$billet)){
		 	...
		 	$buffer_template_HTML=str_replace($billet,$page,$buffer_template_HTML);
 		}
	}
...
//==================================================================================================
	...
	$buffer_template_HTML=str_replace("#CONTENU#",$tableglobal,$buffer_template_HTML);
	$buffer_template_HTML=str_replace("#SOUSCATEGORIE#",$souscat,$buffer_template_HTML);
	$buffer_template_HTML=str_replace("#CALENDRIER#",$calendrier,$buffer_template_HTML);
	$buffer_template_HTML=str_replace("#NEWSLETTER#",$newsletter,$buffer_template_HTML);
	$buffer_template_HTML=str_replace("#META#",$METATAGS,$buffer_template_HTML);
	$buffer_template_HTML=str_replace("#CHEMINDEFER#",$cheminfer,$buffer_template_HTML);
	$buffer_template_HTML=str_replace("#LANGUE#",$langueFORM,$buffer_template_HTML);
