...
class billet{
	...
	function rowmenu($row,$count,$ul,$page,$lettre,$orderby,$search,$famille,$GRANDparent){
		...
		/*** si il n'y a pas de billet associ� il peut etre effacer sans probleme ***/
		...
		switch($type){
			...
			case "date":				
				...
				if ($option=="limite"){
					...
					$nom="visible � partir du ".decodedate($date_debut);
				}
				break;	
			case "gallerie":
			...
			case "texte":
				switch($option){					
					...
					case "left":
						...
						$alt="texte fer � gauche";	
						break;
					case "right":
						$pic="ico_texte_alignright.gif";
						$alt="texte fer � droite";	
						break;
					case "centre":
	...
	function legende(){		
		...
		$legende['option']['texte']=array(	
			...
			'categorie/page mise en premi�re page',
			'categorie/page pas mis en premi�re page',
		);
		//------------------------------------------
		// CATEGORIE
		//------------------------------------------
		...
		$legende['paragraphe']['texte']=array(	
			'Titre du pragraphe',
			'Image align�e � gauche dans le paragraphe',
			'Image centr�e dans le paragraphe',
			'Image align� � droite dans le paragraphe'
		);
