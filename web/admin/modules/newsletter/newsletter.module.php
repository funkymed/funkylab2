...
class newsletter
{
	...
	function listmail(){
		...
		<table align=\"center\" cellspacing=\"0\" cellpadding=\"0\" class=\"loginbox\" width=\"250\">
			...
			<tr>
				<td align='right'>ALERTE EMAIL ERRONN� &nbsp;</td><td width=\"10\" class='ALERTEMAIL'></td>
			</tr>		
			<tr>
				<td align='right'>ALERTE IP REPET�E &nbsp;</td><td width=\"10\" class='ALERTEIP'></td>
			</tr>						
		</table>";
	...
	function linewindows($id,$mail,$date,$IPcreation,$cat,$color,$error){
		...
		if ($error==null){
			...
			<TD>$mail</TD>
			<TD>",decodedate($realdate[0])," � ",$realdate[1],"</TD>
			<TD>$IPcreation</TD>
			<TD >$delete</TD>
		</TR>";
	}	
		...
		<FORM ACTION='index.php' method='GET'>
			<TABLE border=0 align='center' cellspacing='0' cellpadding='0' width=100%>
				...
				{      	  				
					...
					$date=decodedate($dateexplode[0])." � ".$dateexplode[1];
					
					if ($senddate!="0000-00-00 00:00:00"){
						$senddateexplode=explode(" ",$senddate);
						$senddate=decodedate($senddateexplode[0])." � ".$senddateexplode[1];
						$file="newsletter_oui.gif";
					}else{
						$file="newsletter_non.gif";
						$senddate="pas encore envoy�e";
					}
