	...
	class comment{
		...
		function checkcomment($add_pseudo,$add_mail,$add_commentaire,$idbillet){	
			...
			$message=array();
		  	...
		  	if($add_pseudo==""){
			  	$error=true;
			  	$message[]="Vous n'avez pas entr� de pseudo";
		  	}
		  	if ($add_mail==""){
			  	$error=true;
			...
			if ($error==true){ 
				$msgtxt.=  "<P class=\"titrebox\">Votre commentaire n'a pas �t� envoy� car il contient des erreurs</P>";			
			}else{
				$msgtxt.=  "<P class=\"titrebox\">votre commentaire est enregistr�</P>";
